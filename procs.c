...
void fillblock(int xx1,int yy1,int xx2,int yy2,int colo,int where)
{
...
void coolbox(int x1,int y1,int x2,int y2,int fill,int page)
{
   int i;
   /*
   for(i=0;i<5;i++) box(x1+i,y1+i,x2-i,y2-i,31-i,page);
   if(fill) for (i=y1+5;i<y2-5;i++) 
...
int showhiscore(int showchart,int savelist)
{
...
{
/*  cls(0,vga);
  coolbox(0,0,319,199,0,vga);
  outtextxy("HELP",144,10,4,0,vga,0);
...
void makehole(int y,int tempaddr)
{
...
int main(int argc,char **argv)
{
   ...
   if (argc>1){ 
      for (i=1;i<argc;i++){
          strcpy(tempst,argv[i]);
          for (j=0;j<strlen(tempst);j++)
              tempst[j]=toupper(tempst[j]);
          if (strstr(tempst,"CDROM")!=NULL) cdrom_mode=1;
          if ((strstr(tempst,"HELP")!=NULL)|| (tempst[1]=='H') ||
	        (tempst[1]=='?')){
             printf("Tom Bombem\n\n");
   ...
   ReadConfigFile();
   axel_und_carmen=1;
      /*{as_of_9-22-94} {change_back_10-6-94} {uh_oh 2-21-95} 
